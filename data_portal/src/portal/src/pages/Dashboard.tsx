...
import { Card, Typography, Row, Col, Modal, Radio, App } from 'antd';
...
import { Server, Database, Activity, ShieldCheck, AlertTriangle, Cpu, Network, Layout, Download, ServerCog, Layers, Search, X } from 'lucide-react';
...
export const Dashboard: React.FC = () => {
  ...
  const [systemInfo, setSystemInfo] = useState({ cpuPercent: 0, memPercent: 0, memUsedGb: 0, memTotalGb: 0 });
  ...
  const handleExportReport = () => {
    ...
    setTimeout(() => {
      // Create simple text report and download
      const now = new Date().toLocaleString('ko-KR');
      const reportContent = [
        '===================================================',
        '  아산 의료 데이터 패브릭 - 플랫폼 현황 리포트',
        '===================================================',
        `생성일시: ${now}`,
        '',
        '■ 시스템 상태',
        `  CPU 사용률: ${systemInfo.cpuPercent.toFixed(1)}%`,
        `  메모리: ${systemInfo.memUsedGb}/${systemInfo.memTotalGb} GB (${systemInfo.memPercent.toFixed(0)}%)`,
        '',
        '■ 데이터 현황',
        `  총 레코드: ${totalRecords.toLocaleString()}건`,
        '',
        '■ 데이터 품질 지수',
        ...qualityData.map(q => `  ${q.domain}: ${q.score}점 (이슈 ${q.issues}건)`),
        '',
        '■ 검사 활동 추이',
        ...activityData.map(d => `  ${d.month}: ${d.count.toLocaleString()}건`),
        '',
        '===================================================',
        '  Generated by Asan IDP Dashboard',
        '===================================================',
      ].join('\n');
      ...
      a.href = url;
      a.download = `dashboard_report_${new Date().toISOString().slice(0,10)}.txt`;
      a.click();
  ...
  // Fetch dashboard stats from OMOP CDM
  ...
  }, [fetchDashboardStats, fetchSystemResources, settings.autoRefresh]);
    ...
    <div className="space-y-6 animate-fade-in text-[#53565A] pb-20">
      ...
      >
        <div style={{ padding: '12px 0' }}>
          ...
          <Radio.Group value={layoutChoice} onChange={e => setLayoutChoice(e.target.value)} style={{ width: '100%' }}>
      ...
      >
        <div style={{ padding: '12px 0' }}>
          <p style={{ color: '#666', marginBottom: 12 }}>현재 대시보드 현황을 텍스트 리포트로 내보냅니다.</p>
