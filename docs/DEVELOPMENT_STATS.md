...
| **QA 테스트** | 358개 (Backend 306 + Frontend 52) |
...
| **QA 테스트 커버리지 확장 (02/11)** | **358개 자동화 테스트** — Backend pytest 306개 + Frontend Vitest 52개 |
| — 신규 테스트 7파일 | `test_bi_shared_helpers`(26), `test_ai_ops_helpers`(35), `test_bi_query`(14), `test_bi_chart`(13), `test_consent_mgmt`(14), `test_governance_ext`(10), `test_ai_ops_api`(15) |
| — 커버리지 설정 | `.coveragerc` — routers/services/middleware 소스, `--cov-report=term-missing` |
...
| — API 라우터 테스트 | health(10), auth(26), datamart(6), text2sql(8), ner(5), presentation(7), conversation(4), bi_query(14), bi_chart(13), consent(14), governance(10), ai_ops(15) |
...
|--------|-----------|----------|----------|
| **Backend 단위** | pytest + pytest-asyncio | **286개** | 12s |
...
| `tests/test_bi_shared_helpers.py` | SQL 검증, LIMIT 자동추가 순수 함수 | 26 |
| `tests/test_ai_ops_helpers.py` | PII 탐지/마스킹, 환각검증, 인젝션 패턴 | 35 |
| `tests/test_bi_query.py` | BI 쿼리 실행/검증/이력/저장 | 14 |
| `tests/test_bi_chart.py` | BI 차트/대시보드 CRUD | 13 |
| `tests/test_consent_mgmt.py` | 동의 정책/동의/철회/감사로그 | 14 |
| `tests/test_governance_ext.py` | 민감도 분류, RBAC 역할 CRUD | 10 |
| `tests/test_ai_ops_api.py` | AI 모델 관리, PII/인젝션/감사 API | 15 |
...
# 전체 커버리지
cd data_portal/src/api && python -m pytest tests/ -m "not integration and not e2e" --cov=routers --cov=services --cov=middleware --cov-report=term-missing -v
```
...
*Generated by Claude Code (Claude Opus 4.6) — 2026-02-11*
